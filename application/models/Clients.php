...
class Application_Model_Clients extends Application_Model_Base {
    ...
    function getArrayData($clientId) {
        $sql = "SELECT  A.*, (SELECT FIRST 1 DESCRIPTION FROM SUPPORT\$COUNTRIES CO
                    WHERE CO.COUNTRY_ID IN
                     (SELECT COUNTRY_ID FROM SUPPORT\$ZIP_CODES Z WHERE Z.ZIP_CODE_ID = A.ZIP_CODE_ID)) AS COUNTRY_DESCRIPTION,
                  B.TRAIN_TYPE,C.COUNTRY_ID,D.COUNTRY_ID AS INVOICE_COUNTRY_ID,B.ARTICLE,B.COURT,B.ACTIVITIES,B.EMAIL_FOR_UPDATES FROM
