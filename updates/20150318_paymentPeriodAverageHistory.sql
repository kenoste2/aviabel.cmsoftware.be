...
INSERT INTO FILES$STATES (STATE_ID, CODE, DESCRIPTION, ACTIEF, DELETEPOSSIBLE)
  VALUES((SELECT MAX(STATE_ID) + 1 FROM FILES$STATES), 'OVER_DELAY', 'payment is past expected delay', 1, 0);
...
INSERT INTO FILES$ACTIONS (ACTION_ID,CODE,DESCRIPTION,AMOUNT,INVOICEABLE,SYSTEM,FIRST_ACTION,COST_ID,VISIBLE,COLLECTOR,ACTIEF,FILE_STATE_ID,DELETEPOSSIBLE)
  VALUES((SELECT MAX(ACTION_ID) + 1 FROM FILES$ACTIONS), 'OVER_DELAY', 'payment is past expected delay', 0.00, 0,0,0,NULL,1,'N',1,(SELECT MAX(STATE_ID) FROM FILES$STATES),0);
